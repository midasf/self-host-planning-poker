...
def check_db_file_permissions():
    ...
    try:
        ...
        print(f"ERROR: User does not have write permissions on {path}.\n" +
              "\tUpdate permissions so that it is owned and writable by 10001:0.\n" +
              "\tIf you migrated from an earlier version, run `chown 10001:0 -R <path_to_data_volume>` (Docker) " +
              "or `podman unshare chown 10001:0 -R <path_to_data_volume>` (Podman)")
